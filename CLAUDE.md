...
- **Specs (read these first):**
  ...
  - `docs/e5063a-gui-spec.md` — the GUI build plan + status. **GUI is BUILT, POLISHED & live-validated: phases G-0…G-5 + G-7…G-11 ✅** (one-stop Configure / Calibrate / Sanity Check / Continuous Monitor; G-7…G-11 = UI polish — see design-system §8/§9). Only **G-6 (`.exe` packaging)** remains. Lives in `code/ena-dev/gui/`.

...
class libreVNA:
    ...
    def close(self):
        """
        Explicitly close the SCPI socket and stop all streaming threads.

        Stops any active live callback threads by clearing their callback
        lists (which causes the thread loop to exit), then joins them with
        a timeout. Finally closes the main SCPI TCP socket.

        Safe to call multiple times.
        """
        # Stop all streaming threads by clearing callbacks
        for port in list(self.live_callbacks.keys()):
            self.live_callbacks[port] = []  # Thread loop checks len() and exits

        # Wait for streaming threads to finish (with timeout)
        for port in list(self.live_threads.keys()):
            try:
                self.live_threads[port].join(timeout=3.0)
            except Exception:
                pass

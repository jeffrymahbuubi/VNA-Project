...
Usage:
    uv run python scan_mcp_servers.py [--config PATH] [--analyzers yara,llm,api] [--output PATH]

Examples:
    # Scan with YARA only (default)
    uv run python scan_mcp_servers.py

    # Scan with YARA and LLM analyzers
    uv run python scan_mcp_servers.py --analyzers yara,llm

    # Scan with custom config file
    uv run python scan_mcp_servers.py --config /path/to/.mcp.json --output results.json
"""
...
try:
    ...
    print("Install it with: uv pip install cisco-ai-mcp-scanner")
    sys.exit(1)
